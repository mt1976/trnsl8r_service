...
func (job *localeUpdate) Run() error {
	...
	logHandler.Service.Printf("[%v] Number of Locales=[%v]", domain.String(), noLocales)

	// Check that the locales follow the structure zz_ZZ, otherwise error out
	for _, locale := range locales {
		logHandler.Service.Printf("[%v] Checking locale formatting for [%v] %v", domain.String(), locale.Key, locale.Name)
		if len(locale.Key) != 5 {
			msg := fmt.Sprintf("locale [%v] is not in the correct format", locale.Key)
			logHandler.Error.Println(msg)
			j.Stop(0)
	...
	logHandler.Service.Printf("[%v] Locales are in the correct format", domain.String())
	// We can assume that all the locales are in the correct format
	textData, err := textStore.GetAll()
	if err != nil {
		logHandler.Error.Println(err)
		j.Stop(0)
		return err
	}
	noText := len(textData)
	logHandler.Service.Printf("[%v] Number of Translations to Verify=[%v]", domain.String(), noText)
	upgradeText := false
	for thisPos, text := range textData {
		if len(text.Localised) != noLocales {
			...
			logHandler.Service.Println(msg)
			upgradeText = true
		}
	...
	logHandler.Service.Printf("[%v] Locales Updated", domain.String())
...
func (job *localeUpdate) Name() string {
	return translate.Get(context.Background(), "Update Locales", "")
}
